  ...
  return {
    ...
    async init() {
      try {
        ...
        await Promise.all([this.loadCategories(), this.loadDashboardData()]);
...
document.addEventListener("click", (event) => {
  const card = event.target.closest(".seller-auction-card");
  ...
  window.location.assign(`/auction-details.html?id=${auctionId}`);
});
